    ...
    :next(function()
      ...
      return Promise.mapSeries(function(item)
        ...
        elseif ftype == 'link' then
          return M.copy_symlink(source, target, { exist_ok = true, keep_times = keep_times })
...
function M.copy_directory(path, new_path, opts)
  if opts and opts.create_symlink then
    local path_symlink_target, errmsg, err = uv.fs_readlink(path)
    if path_symlink_target then
...
function M.remove_directory(path, opts)
  ...
  if opts and opts.recursive then
    ...
    local rm_dirs = Promise.mapSeries(function(subpath)
      return M.remove_directory(subpath, opts)
    end, dirs)
    local rm_files = Promise.mapSeries(function(subpath)
