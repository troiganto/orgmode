...
---This simply uses `fs_scandir()` (as it is slightly faster than `fs_stat()`),
---so it always resolves symlinks.
---@param path string
---@return boolean result
function M.is_dir(path)
  local dir, errmsg, err = uv.fs_scandir(path)
  assert(err == 'ENOTDIR' or err == 'ENOENT', errmsg)
  return dir and true or false
end
...
function M.is_symlink(path)
  local link, errmsg, err = uv.fs_readlink(path)
  assert(err == 'EINVAL' or err == 'ENOENT', errmsg)
...
local function groupdir(path)
  ---@todo is scandir or readdir the faster option?
  local node = assert(uv.fs_scandir(path))
  local res = {}
  while true do
...
local function copy_stats(path, new_path, keep_times)
  local stat, errmsg, err = assert(uv.fs_stat(path))
...
function M.copy_directory(path, new_path, opts)
  ...
  if path_symlink_target and create_symlink then
    ...
    return M.symlink(path_symlink_target, new_path, { dir = true, junction = true, exist_ok = true }):next(function()
      return true
    end)
  end
...
function M.remove_directory(path, opts)
  ...
  if opts and opts.recursive then
    ...
    items.directory = nil
    local rest = vim.tbl_flatten(vim.tbl_values(items))
